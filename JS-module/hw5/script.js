...
let a1 = new Tags('a', '<a> предназначен для создания ссылок',[{title:'href',
    action: 'Задает адрес документа, на который следует перейти'},
    {title:'target', action:'Имя окна или фрейма, куда браузер будет загружать документ'}]);
console.log(a1);

let div1 = new Tags('div', ' <div> является блочным элементом', [{title:'align',
...
let span1 = new Tags('span','Тег <span> предназначен для определения строчных элементов документа',
    [{title:'dir', action:'Задает направление и отображение текста — слева направо или справа налево'},
        {title:'class',action:'Определяет имя класса'}]);
console.log(span1);

let input1 = new Tags('input','<input> предназначен для создания текстовых полей, различных кнопок, переключателей и флажков',
    [{title:'accept', action:'Устанавливает фильтр на типы файлов, которые вы можете отправить через поле загрузки файлов'},
        {title:'value', action:'Значение элемента'}]);
console.log(input1);
...
let option1 = new Tags('option','Тег <option> определяет отдельные пункты списка, создаваемого с помощью контейнера <select>',
    [{title:'disabled', action:'Заблокировать для доступа элемент списка'},
        {title:'label', action:'Указание метки пункта списка'}]);
console.log(option1);
...
let select1 = new Tags('select','Тег <select> позволяет создать элемент интерфейса в виде раскрывающегося списка',
    [{title:'form', action:'Связывает список с формой'},
        {title:'required',action:'Список обязателен для выбора перед отправкой формы'}]);
console.log(select1);
